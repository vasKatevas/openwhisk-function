...
test_commands () {
  ...
  while [ ! -z "$(kubectl get pods -n openwhisk | grep PodInitializing)" ]; do sleep 30s; done
...
write_logs_to_file () {
  ...
  echo "$memorySize,$(echo $userMemory | sed -e 's/m//g'),$(echo $minMemory | sed -e 's/m//g'),$(echo $maxMemory | sed -e 's/m//g'),$averageWaitTime,$averageUserSideDelay,$averageStartLatency,$averageInitTime,$averageDuration,$achievedAverageRate,$stdDevDuration,$stdDevInitTime,$stdDevStartLatency,$stdDevUserSideDelay,$stdDevWaitTime,$successPercentage,$coldStarts" >> results.csv
}
